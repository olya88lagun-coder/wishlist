...
> **Статус: выполнен 2026-09-15.** Прод: https://my-wish-list.online — списки, подарки, бронирование гостем и режим сюрприза проверены вручную.
> Отличия от текста задач: формы возвращают отправленные значения (`FormState.values`), чтобы сброс формы в React 19 не стирал ввод после ошибки; пул БД хранится в `globalThis` (одно соединение на процесс для локальной PGlite); `ShareBar` проверяет `navigator.share` после гидратации.
