...
- [x] **Step 2: Бакеты и ключи S3 (делает пользователь)**
...
- [x] **Step 3: Мерж в master и сборка образов (с согласия пользователя)**
...
- [x] **Step 4: Окружение на сервере (пользователь + «да»)**
...
- [x] **Step 5: Деплой (только после «да»)**
...
- [x] **Step 6: Бэкап и публичный URL фото (только после «да»)**
...
4. Ссылка на Яндекс Маркет (карточка `/card/...`) → фото, название и цена из JSON-LD карточки, без подсказок. Если карточка без JSON-LD-цены — подсказка «Яндекс Маркет не отдал цену — впишите её»; вписать цену через «Изменить» → подсказка исчезла.
...
```
> **Статус: выполнен YYYY-MM-DD.** Прод: https://my-wish-list.online — автозаполнение по ссылке (WB, Золотое Яблоко, Яндекс Маркет — ok; Ozon — partial), фото в S3, ночной бэкап проверены вручную.
```
